...
for encoder in "${encoders[@]}"
    ...
    for encoder2 in "${encoders2[@]}"
        do
            ...
            python ../train_fit.py -exp ${expname} -s_last -arc BiLSTMLateFusion -enc ${encoder} -enc2 ${encoder2} -lr 1e-3 -hs -huss 256 -nlss 2 -diss 0 0.2 0.5 -doss 0 0.2 0.5 -data NonNews -bs ${bs} -ef ${encoder} -ef2 ${encoder2} -lf NonNewsSentence/labs_dict.pkl --metric ${metric} -max 1000 -vp 0.15 -pat 50 -ar -as -split NonNews_split.json -loss FocalLoss
        done
	counter+=1
    done