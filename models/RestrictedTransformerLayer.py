...
from transformers import LongformerConfig, LongformerModel, BertConfig, BertModel, LongT5Config, LongT5EncoderModel

from models.longformer_noffn import LongformerLayer
...
class Longformer_Local_Attention(nn.Module):
    def __init__(self, d_model: int, nhead: int, n_layers:int, 
                dim_feedforward: int = 2048, 
                 ...
                 layer_norm_eps: float = 1e-5,
                 tagset_size: int = 2,
                 device=None, max_position_embedding: int = 4096, just_mha = False):
        ...
        self.configuration.num_labels = tagset_size 
        self.configuration.max_position_embeddings = max_position_embedding
        self.configuration.attention_probs_dropout_prob = dropout_attention
        
        if just_mha:
            self.model = LongformerLayer(self.configuration)
            self.layer = True
        else:
    ...
    def forward(self, inputs, lengths, external_input = None):
        device = inputs.device
        ...
        if self.layer:
            mask-=1
            out = self.model(hidden_states = inputs, attention_mask = mask, external_input = external_input)
...
class LongT5(nn.Module):
    def __init__(self, d_model: int, nhead: int, n_layers:int, 
                dim_feedforward: int = 2048, 
                 ...
                 layer_norm_eps: float = 1e-5,
                 tagset_size: int = 2,
                 device=None, bucket_position_embedding: int = 32):
        
        super(LongT5, self).__init__()
                     
        self.configuration = LongT5Config()
        
        self.configuration.local_radius = window_size
        self.configuration.dropout_rate = dropout
        self.configuration.num_layers = n_layers
        self.configuration.d_model = d_model
        self.configuration.d_ff = dim_feedforward
        self.configuration.num_heads = nhead
        #self.configuration.relative_attention_num_buckets = bucket_position_embedding
        self.configuration.relative_attention_num_buckets = max(4, window_size+1//4)
        self.configuration.relative_attention_max_distance = window_size+1
        
        self.model = LongT5EncoderModel(self.configuration)
